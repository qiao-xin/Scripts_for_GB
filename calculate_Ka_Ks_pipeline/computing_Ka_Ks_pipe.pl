...
sub usage{
	die "Usage: ", basename($0)," -d <CDS.fasta> -g <Gene_pairs> -o outfile.axt\n";
}
...
#my ($sp) = ($gene_file =~ /^(\S+?)\./);
system "cat $out_dir/*.axt > $out_file";
...
system "KaKs_Calculator -i $out_file -m GMYN -o $out_file.kaks > $out_file.KaKs_Calculator.log";

open IN, "$out_file.kaks";
open OUT, ">$out_file.kaks.simplified";
